...
void Line::prePrintTKZ(){
	if( !isVisited ){
		...
		if( midOrthL != 0 ){
			cout << "\\tkzMediatorLine[prefix=g,kr=0.7,kl=0.8,color=blue](" << endP1->getLabel()
				<< "," << endP2->getLabel() << ")" << endl;
			cout << "\\tkzGetPointxy(gl){p}" << endl;
			cout << "\\tkzPoint*(\\px,\\py)" <<"{"<< midOrthL->getEndP(1)->getLabel()
				<< "}" << endl;
			cout << "\\tkzGetPointxy(gr){q}" << endl;
			cout << "\\tkzPoint*(\\qx,\\qy)" <<"{"<< midOrthL->getEndP(2)->getLabel()
				<< "}" << endl;
			midOrthL->getEndP(1)->setVisit( true );
