...
void LCRelationTg::prePrintTKZ(){
	...
	if( c->getR() > 0 ){
		...
		tgP->setVisit( true );
	}else if( c->getDiameter() != 0 ){
		c->getDiameter()->prePrintTKZ();
		cout << "\\tkzMidPoint*(" << c->getDiameter()->getEndP(1)->getLabel() << "," 
			<< c->getDiameter()->getEndP(2)->getLabel() << "){" << "t}" << endl;
		cout << "\\tkzMathLength(" << c->getDiameter()->getEndP(1)->getLabel() << ","
			<< c->getDiameter()->getEndP(2)->getLabel() << ")" << endl;
		cout << "\\tkzTgtFromPR(" << c->getC()->getLabel()
			<< ",\\tkzmathLen pt)(" << l->getEndP(1)->getLabel() << "){"
			<< tgP->getLabel() << "}{" << "NULL}" << endl;
		cout << "\\tkzVectorKLinear[k=0.3](" << l->getEndP(1)->getLabel() << ","
			<< tgP->getLabel() << "," << tgP->getLabel() << "){" << l->getEndP(2)->getLabel()
			<< "}" << endl;
